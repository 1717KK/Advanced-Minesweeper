...
def mousePressed(event, data):
    ...
    # start game again
        ...
        startGameAgain(event, data)
            
def startGameAgain(event, data):
        ...
        if event.keysym == "Up":
            ...
            if data.useBomb and data.numOfBombs > 0:
                ...
                for d in direction:
                    ...
                    if 0 <= newRow <= 9 and 0 <= newCol <= 19:
                        data.initMap[newRow][newCol] = -1
                        data.newMap3[newRow][newCol] = -1
                        if (newRow, newCol) in data.clue:
                            del data.clue[newRow, newCol]
                data.initMap[0][0] = 0
                data.initMap[9][19] = 0
                data.useBomb = False
...
def rule(canvas, data):
    canvas.create_rectangle(0, 0, data.width, data.height, \
                            fill = "black")
    pos1 = (850, 35)
    canvas.create_text(pos1[0], pos1[1], text = "Game Rules", fill = "white", \
                        font = ('Helvetica', '30')) 
    rules = "Goals:\n- Collect coins (Press 'c') as much as possible and "+\
            "escape the maze in the shortest time\n\nHeart: \n"+\
            "- The explorer will have 5 hearts at the beginning of"+\
            " game.\n"+\
            ...
            "Map:\n- The explorer won't have maps at the beginning of"+\
            " game.\n"+\
            "- If the explorer collects more than 5 coins, he will"+\
    ...
    pos2 = (500, 300)
    canvas.create_text(pos2[0], pos2[1], text = rules, fill = "white", \
                        font = ('Garamond', '15'))  
    pos3 = (950, 550)
    r = 20
    canvas.create_oval(pos3[0] - r, pos3[1] - r, pos3[0] + r, pos3[1] + r, \
                        outline = "white", width = 2)
    pos4 = (940, 550)
    pos5 = (960, 540)
    pos6 = (960, 560)
    canvas.create_line(pos4[0], pos4[1], pos5[0], pos5[1], \
                        fill = "white", width = 2) 
    canvas.create_line(pos4[0], pos4[1], pos6[0], pos6[1], \
                        fill = "white", width = 2) 
  
# draw information board for each level
def drawInfo(canvas, data):
    pos1 = (400, 300)
    canvas.create_text(pos1[0], pos1[1], \
                        text = " Time: \n Coin gathered: \n" + \
                        " Monster defeated: \n Level reached:", \
                        fill = "white", font = ('Garamond', '20'))
    pos2 = (650, 300)
    canvas.create_text(pos2[0], pos2[1], text = str(data.time) + "\n" + \
                        str(data.numOfCoins) + "\n" + \
                        str(data.monsterDefeated) + "\n" \
                        + str(data.level), fill = "white", \
                        font = ('Garamond', '20'))
    pos3 = (305, 450, 455, 500)
    canvas.create_rectangle(pos3[0], pos3[1], pos3[2], pos3[3], \
                            outline = "white", width = 1)
    pos4 = (505, 450, 655, 500)
    canvas.create_rectangle(pos4[0], pos4[1], pos4[2], pos4[3], \
                            outline = "white", width = 1)
    pos5 = (580, 475)
    canvas.create_text(pos5[0], pos5[1], text = "Home", fill = "white", \
                        font = ('Helvetica', '15'))
    ...
    if not data.startGame:
        ...
        if data.gameOver:
            canvas.create_rectangle(0, 0, data.width, data.height, \
                                    fill = "black")    
            pos1 = (300, 100)
            canvas.create_text(pos1[0], pos1[1], text="Better Luck Next Time", \
                               fill = "white", font = ('Helvetica', '30'))                                           
            drawInfo(canvas, data)
            pos2 = (380, 475)
            canvas.create_text(pos2[0], pos2[1], text = "New Game", \
                                fill = "white", font = ('Helvetica', '15'))
        elif data.win:
            canvas.create_rectangle(0, 0, data.width, data.height, \
                                    fill = "black")  
            pos1 = (300, 100)
            canvas.create_text(pos1[0], pos1[1], text = "Congratulation", \
                               fill = "white", font = ('Helvetica', '30'))                                           
            drawInfo(canvas, data)
            pos2 = (380, 475)
            canvas.create_text(pos2[0], pos2[1], text = "Next Level", \
                                fill = "white", font = ('Helvetica', '15'))
        else:
            drawMap(data, canvas)
            ...
            for bullet in data.bullets:
                bullet.draw(canvas)
                if bullet.isOffScreen(maxX, maxY):
                    data.bullets.remove(bullet)
            for monster in data.monsters:
                monster.draw(canvas)
            for i in range(data.index):
